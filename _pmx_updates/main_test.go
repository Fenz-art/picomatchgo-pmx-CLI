...
func TestPMXDoctorFixturePath(t *testing.T) {
	...
	if got := report["summary"].(map[string]interface{})["warn"]; got.(float64) < 3 {
		t.Fatalf("warn summary = %v; want at least 3", got)
	}
...
func TestPMXDoctorBrokenFixtureJSONContract(t *testing.T) {
	...
	// All diagnostics in the broken fixture should be warnings (not failures)
	for _, item := range diagnostics {
		d := item.(map[string]interface{})
		if got := d["severity"]; got != "warn" && got != "pass" {
			t.Fatalf("diagnostic severity = %v; want warn or pass: %v", got, d)
		}
	}
